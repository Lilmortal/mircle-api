...
public class UserControllerTest {
  ...
  @Captor private ArgumentCaptor<User> updatedUserCaptor;
  ...
  public void givenIdThenReturnUser() throws Exception {
    ...
    MvcResult result =
        mvc.perform(get(String.format("/user/%d", ID)).contentType(MediaType.APPLICATION_JSON))
  ...
  public void givenEmailAddressThenReturnUser() throws Exception {
    ...
    MvcResult result =
        mvc.perform(
                get(String.format("/user/email/%s", EMAIL_ADDRESS))
  ...
  public void givenInvalidIdWhenFindingUserShouldThrowUserNotFound() throws Exception {
    ...
    mvc.perform(get(String.format("/user/%d", ID))).andExpect(status().is5xxServerError());
  }
  ...
  public void givenPhoneNumberThenUpdateUser() throws Exception {
    ...
    mvc.perform(
            patch(
                String.format("/user?emailAddress=%s&phoneNumber=%s", EMAIL_ADDRESS, phoneNumber)))
        .andExpect(status().isOk())
        .andReturn();
  ...
  public void givenBirthDateThenUpdateUser() throws Exception {
    ...
    mvc.perform(
            patch(String.format("/user?emailAddress=%s&birthDate=%s", EMAIL_ADDRESS, birthDate)))
        .andExpect(status().isOk())
        .andReturn();
  ...
  public void givenOccupationThenUpdateUser() throws Exception {
    ...
    mvc.perform(
            patch(String.format("/user?emailAddress=%s&occupation=%s", EMAIL_ADDRESS, occupation)))
        .andExpect(status().isOk())
        .andReturn();
  ...
  public void givenIdThenUpdateUserPassword() throws Exception {
    ...
    mvc.perform(
            patch(
                String.format(
                    "/user/password?id=%d&oldPassword=%s&newPassword=%s",
  ...
  public void givenEmailAddressThenUpdateUserPassword() throws Exception {
    ...
    mvc.perform(
            patch(
                String.format(
                    "/user/password?emailAddress=%s&oldPassword=%s&newPassword=%s",
                    EMAIL_ADDRESS, PASSWORD, newPassword)))
        .andExpect(status().isOk())
        .andReturn();
  }
  ...
  public void givenIdAndNoProfileImageThenUpdateUserProfileImageToDefault() throws Exception {
    ...
    mvc.perform(post(String.format("/user/profileimage?id=%d", ID))).andExpect(status().isOk());
  }
  ...
  public void givenEmailAddressAndProfileImageThenUpdateUserProfileImage() throws Exception {
    ...
    when(userService.setUserProfileImage(user, profileImageUrl)).thenReturn(user);
    when(profileImageService.uploadProfileImageToS3(file, EMAIL_ADDRESS))
        .thenReturn(profileImageUrl);
    when(userService.setUserProfileImage(eq(user), urlCaptor.capture())).thenReturn(user);

    mvc.perform(
            MockMvcRequestBuilders.fileUpload("/user/profileimage")
                .file(file)
  ...
  public void givenIdThenDeleteUser() throws Exception {
    ...
    mvc.perform(delete(String.format("/user/%d", ID))).andExpect(status().isOk()).andReturn();
  }
